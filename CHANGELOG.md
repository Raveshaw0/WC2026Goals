...
## a6e6b14, abfd129, a9d869d - SBS highlights matching and the in-game reel

Two SBS highlight bugs, both found and fixed while the semifinals were live.

**Wrong YouTube highlights video (a6e6b14).** The embedded post-match highlight sometimes belonged to a different game (the England v Argentina semi showed a video of the two teams' separate quarter-finals). The matcher accepted any video whose title mentioned both team names plus "highlights", so a multi-game roundup ("FIFA Highlights Show | Quarter-finals: Norway v England, Argentina v Switzerland") cross-matched the semi, since both teams are named in it from different games. It also let an earlier-in-feed roundup outrank the dedicated per-match clip, so two different fixtures ended up sharing one video (the two quarter-finals in that roundup). The matcher now requires the two teams as an adjacent "A v B" pair (`titleHasFixture` in `aliases.ts`), which rejects roundups, then ranks candidates by fewest versus-pairs (a dedicated single-match clip over a roundup) and a plain highlights cut over the "Highlights Show" recap. Verified against the live feed: the three affected fixtures each resolved to their own clip, and six `sbs_links` rows corrected on the next check.

**In-game clip reel froze mid-match (a9d869d, abfd129).** The Blaze clip reel stopped growing partway through a game (it "found 4-5 then stopped"). Two causes. The primary one (a9d869d) was server-side: the Blaze feed fetch used Next's Data Cache (`revalidate: 60`), and that entry froze at an early-match snapshot, measured in production as 5 clips with `isLive` true, hours after full time, while the raw feed had 16 and was not live; six straight requests never revalidated. Not the 2MB size cap (the feed is ~1.5MB); the exact internal reason is unconfirmed. Switched to `cache: "no-store"` so every poll fetches fresh, verified in prod flipping from the frozen 5 to the full 16. The secondary cause (abfd129) was client-side: the clip poll shared the summary poll, which only ran inside the live window and died at status "finished" and no-ops while the tab is hidden, so switching to the SBS stream skipped every tick and returning after full time hit a `visibilitychange` refetch gated on the now-closed window. Clips now have their own refresh window (75 minutes pre-kickoff until the official highlight embeds or 6h past kickoff), independent of the live window and the finished status, and the visibility refetch is gated on the union so returning from the SBS stream still pulls new clips.

## 60bae31, e136cea, c386a8a - SBS link discovery from match pages, 24h backstop

SBS and YouTube links were not appearing for the 3rd place playoff (France v England) for hours after its highlights posted. Measured: the links were already in the feeds and the matcher matched them (the dedicated clip `DektEsxeG58`, correctly ranked over the roundup), but the stored row was stale, only the live link, last checked hours earlier; a manual discovery pass populated everything correctly. So discovery and matching were fine. The cause was cadence: discovery (`/api/check-sbs`) only fired from the home page, plus a GitHub Action limited to 08:00 to 19:45 UTC. The playoff kicked off 21:00 UTC and its highlights posted overnight UTC, entirely outside the Action window, and match pages never triggered discovery, so sitting on the match page pulled nothing.

Two fixes. The match page now fires the same lazy, self-throttled `/api/check-sbs` POST the home page does, so opening any match refreshes its own links regardless of the Action window. And the Action cron widened from `*/15 8-19 * * *` to `*/15 * * * *` (24h) so the backstop covers overnight-UTC matches (the final kicks off 19:00 UTC, highlights landing after the old cutoff). The workflow also moved from the old manual-paste workaround (`deploy/sbs-links.yml`, pasted via the web UI because the PAT lacked `workflow` scope) to a tracked, auto-deploying `.github/workflows/sbs-links.yml`; the PAT has `workflow` scope now, confirmed by the push going through.
