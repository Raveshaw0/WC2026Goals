...
The embedded highlights player on match detail pages comes from SBS Sport's YouTube channel (@SBSSportau). Discovery parses the `ytInitialData` JSON inside the channel's Videos page, currently shaped as `lockupViewModel` items. YouTube reshapes this periodically (it changed from `videoRenderer` to `lockupViewModel` sometime before June 2026). Matching requires the two team names as an adjacent "A v B" pair plus the word "highlights" (not merely both names anywhere: that used to let a multi-game roundup like "FIFA Highlights Show | Quarter-finals: Norway v England, Argentina v Switzerland" cross-match the England v Argentina game, since both those teams are named in it from separate games; it also let a roundup outrank the real per-match clip, so two fixtures shared one video). Candidates are ranked so a dedicated per-match clip beats a roundup or a "Highlights Show" recap. SBS retitling for knockout rounds will not break it, but a YouTube markup change would. If embeds stop appearing for new matches while SBS links still work, the parser in `fetchYoutubeHighlights()` is the suspect. The channel only lists ~30 recent uploads, so discovery must run within a few days of each match (the 15-minute GitHub Action makes this a non-issue in practice). Stored ids are never deleted.
...
The feed is fetched with `cache: "no-store"`. It previously used a `next: { revalidate: 60 }` Data Cache entry, which was measured in production **frozen** at an early-match snapshot (5 clips, `isLive` true) that never revalidated for hours (six consecutive requests all returned the same stale set, while the raw feed had the full 16 and was not live). That is what stopped the in-game reel growing partway through a match. It was **not** Next's 2MB Data Cache size cap: the feed is ~1.5MB. The exact reason the entry never revalidated is unconfirmed; `no-store` sidesteps the cache entirely, and since the route is force-dynamic and polled ~60s, fetching the ~1.5MB feed fresh each time is negligible. One thing still unmeasured: whether the Blaze list endpoint truncates a story's pages while it is genuinely `isLive`, since no match was live to test after the fix; the next live match will settle it.
