...
The per-match goal/moment clips (what LiveScore syndicates) come from SBS's "Blaze" stories platform, not the catalogue or YouTube. `src/lib/clips.ts` fetches `blazesdk-prod-cdn.clipro.tv/api/blazesdk/v1.3/stories?ApiKey=<public>&labelsFilterExpression=aa-sbs-aus-wc26&maxItems=50` (works server-side, no referer). `result[]` is one story per match (`title` "Brazil vs. Morocco", `description` carrying date/group, `isLive`, `pages[]` clips). Each clip's `pages[].baseLayer.content.renditions[]` is a plain vertical MP4 on sbs.com.au (no DRM, range-supported, plays in our own `<video>`). Stories map to fixtures by team aliases (title split on " vs. ") **plus the match date** (description `dd-mm-yy` within ~1 day of kickoff UTC), which both disambiguates repeated fixtures and excludes the 2022 demo stories sitting in the feed. The SBS editorial GraphQL (`cms.sbs.com.au/graphql/delivery/sbscontentapi`) was introspected and ruled out, it only carries news/feature videos. Clips ride the match poll via `/api/match/[id]`, and the feed is fetched uncached (`cache: "no-store"`): a `next: { revalidate }` Data Cache entry was measured in production frozen at a stale mid-match snapshot, so the reel stopped growing partway through a game (see KNOWN_ISSUES.md).

### Title matching

Rail and video titles match a fixture when the two teams appear as an adjacent "A v B" pair (`titleHasFixture` in `src/lib/aliases.ts`, alias-aware: Korea Republic vs South Korea, Turkiye vs Turkey, Cote d'Ivoire vs Ivory Coast, etc.), plus the word "highlights" for YouTube. Requiring the pair rather than both names anywhere is what rejects multi-game roundup titles ("Quarter-finals: Norway v England, Argentina v Switzerland") that name two teams from different games; among the matches, the fewest versus-pairs and a plain cut over a "Highlights Show" recap win, so a dedicated per-match clip beats a roundup. Still no dependence on the surrounding title text, so knockout-round retitling cannot break it. The alias map was validated against SBS's own teams collection: all 42 announced teams resolve.
...
- Match page: the same 4s score poll, plus `/api/match/[id]` every 60s. The summary (events, stats, lineups) polls from kickoff minus 75 minutes (lineups publish ~1hr out) until the live window closes; the in-game clips poll on their own window (same start, but continuing until SBS's full highlight video embeds or 6h past kickoff) so the reel keeps filling during **and after** the match, since SBS publishes clips past full time. An immediate refresh on `visibilitychange` covers whichever is still live, so returning from the SBS stream pulls fresh clips
...
`src/lib/bracket.ts` (`buildBracket`, a pure transform over `Match[]`, importable on server and client) turns the flat match list into a bracket tree. ESPN tags every knockout match by round and fills the two sides with real teams once decided or placeholder names ("Round of 16 1 Winner") while undecided, but it never returns the bracket **linkage** (which match feeds which). That wiring is fixed for the tournament, so it's encoded once here: feeder maps keyed by each round's date-order slot (`R16-1` fed by `R32-1` and `R32-4`, up through `SF-1` and the final), reconstructed from the live feed (R32 winners traced into their R16 slots by team identity; the upper rounds read off ESPN's placeholder names) and confirmed against the official schedule. ESPN matches bind into slots by round plus kickoff order; each slot carries its bound match and its two feeder slot ids.

`verifyStructure` re-checks the whole chain on every build: for any slot whose match and both feeders have finished with real teams, the slot's two teams must be the winners (losers, for third place) of its feeders. A mismatch flips `consistent` false, which surfaces a warning line in the UI rather than drawing a wrong tree, so a bad map or an ESPN renumbering can't pass silently.

`src/components/BracketTree.tsx` renders it as a two-sided converging tree (page `src/app/bracket/page.tsx`, `force-dynamic`). It re-derives from `useLiveMatches` so scores move live. Layout is pure CSS: columns are flex, each cell is `flex-1` in a fixed-height row, so a match auto-centres on the midpoint of its two feeders; connector lines (`Connectors`) reach half of the 32px column gap from each side so a feeder's forward stub and its child's back stub meet in the middle, with a vertical joining each feeder pair (half drawn from the top cell, half from the bottom). No-spoilers gates advancement: a slot only shows a real team once the feeding match is revealed, otherwise it shows the feeder placeholder, and scores use the shared `SpoilerCover`.
